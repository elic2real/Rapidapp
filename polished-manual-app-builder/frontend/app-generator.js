...
export { ToolRunner };
// Stub PlannerOrchestrator for tests
...
module.exports.PlannerOrchestrator = PlannerOrchestrator;
...
export class AppGenerator {
  ...
  async generateNextJSApp(appDir, appSpec, dbType = 'postgres') {
    ...
    await this.writeFile(
      appDir,
      'package.json',
      this.templateEngine.generatePackageJson(appSpec, 'nextjs', dbType),
    );
    ...
    await this.writeFile(
      appDir,
      'tailwind.config.js',
      this.templateEngine.generateTailwindConfig(),
    );
    ...
    await this.writeFile(
      appDir,
      'src/app/globals.css',
      this.templateEngine.generateGlobalCSS(appSpec),
    );
  ...
  async generateFastAPIApp(appDir, appSpec) {
    ...
    await this.writeFile(
      appDir,
      'requirements.txt',
      this.templateEngine.generateRequirements(appSpec),
    );
    await this.writeFile(appDir, 'app/main.py', this.templateEngine.generateFastAPIMain(appSpec));
    await this.writeFile(
      appDir,
      'app/core/config.py',
      this.templateEngine.generateFastAPIConfig(appSpec),
    );
    ...
    for (const entity of appSpec.databaseEntities) {
      await this.writeFile(
        appDir,
        `app/models/${entity.toLowerCase()}.py`,
        this.templateEngine.generatePydanticModel(entity, appSpec),
      );
      await this.writeFile(
        appDir,
        `app/api/${entity.toLowerCase()}.py`,
        this.templateEngine.generateFastAPIRoute(entity, appSpec),
      );
    }
  ...
  async generateVueApp(appDir, appSpec) {
    ...
    await this.writeFile(
      appDir,
      'package.json',
      this.templateEngine.generatePackageJson(appSpec, 'vue'),
    );
    ...
    for (const page of appSpec.pages) {
      await this.writeFile(
        appDir,
        `src/views/${page}.vue`,
        this.templateEngine.generateVuePage(page, appSpec),
      );
    }
  ...
  async generateRustApp(appDir, appSpec) {
    await this.createDirectoryStructure(appDir, ['src', 'src/handlers', 'src/models', 'src/db']);
    ...
    for (const entity of appSpec.databaseEntities) {
      await this.writeFile(
        appDir,
        `src/models/${entity.toLowerCase()}.rs`,
        this.templateEngine.generateRustModel(entity, appSpec),
      );
    }
  ...
  async generatePage(appDir, pageName, appSpec) {
    ...
    await this.writeFile(
      appDir,
      `${pageDir}/page.tsx`,
      this.templateEngine.generatePage(pageName, appSpec),
    );
  }
  ...
  async generateDatabaseSchema(appDir, appSpec, dbType) {
    if (dbType === 'postgres') {
      await this.writeFile(
        ...
        this.templateEngine.generatePrismaSchema(appSpec),
      );
    } else if (dbType === 'mongodb') {
      await this.writeFile(
        appDir,
        'src/lib/mongodb.ts',
        this.templateEngine.generateMongoSchema(appSpec),
      );
    }
  }
  ...
  async generateAPIRoutes(appDir, appSpec) {
    ...
    for (const entity of appSpec.databaseEntities) {
      ...
      await this.writeFile(
        appDir,
        `${entityDir}/route.ts`,
        this.templateEngine.generateAPIRoute(entity, appSpec),
      );
    }
    ...
    if (appSpec.authentication) {
      ...
      await this.writeFile(
        appDir,
        `${authDir}/route.ts`,
        this.templateEngine.generateAuthAPI(appSpec),
      );
    }
  }
