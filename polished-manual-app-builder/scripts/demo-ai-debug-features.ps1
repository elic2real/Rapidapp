...
if (Test-Path $ExtensionPath) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "⚠️ TypeScript has compilation issues" -ForegroundColor Yellow
        }
        Pop-Location
    } catch {
        Pop-Location
        Write-Host "❌ Could not check TypeScript compilation" -ForegroundColor Red
    }
}
...
$OptionalPackages = @("ollama", "openai", "anthropic", "aiohttp", "fastapi")

Write-Host "Core Python modules:" -ForegroundColor Gray
foreach ($Package in $RequiredPackages) {
    try {
        $TestImport = "import $Package; print('✅ $Package available')" | python 2>$null
        if ($LASTEXITCODE -eq 0) {
...
Write-Host "Optional AI packages:" -ForegroundColor Gray
